...
//hex
using FaceHX = std::tuple<int, int, int, int>;
using CellHX = std::tuple<int, int, int, int,int, int, int, int>;
...
//hex
std::shared_ptr<Mesh> load_hex(std::string filename, bool shuffle) {
  std::cout<<"load_hex"<<std::endl;
  if (filename.substr(filename.size() - 5) == ".node") {
    filename = filename.substr(0, filename.size() - 5);
  }
  FILE *node_fp = fopen((std::string(filename) + ".node").c_str(), "r");
  ...
  int _0, _1, _2, _3, _4,_5,_6,_7,_8; // placeholder

  fscanf(node_fp, "%d %d %d %d\n", &_0, &_1, &_2, &_3);//read node _0 is the number of nodes 
  //_1 is 3 for tetrahedron
  
  auto mesh = std::make_shared<Mesh>(MeshTopology::Hexahedron);
...
//nodes have been read
  ...
  int num_edges = 0;
  auto &EV = mesh->alloca_rel(MeshRelationType::EV);//relation in the file is vertex to elem
  ...
  std::map<FaceHX, int> f_mp;
  auto add_face = [&](int v0, int v1, int v2, int v3) {
    if (f_mp.find(FaceHX(v0, v1, v2,v3)) == f_mp.end() &&
        f_mp.find(FaceHX(v0, v1, v3,v2)) == f_mp.end() &&
        f_mp.find(FaceHX(v0, v2, v1,v3)) == f_mp.end() &&
        f_mp.find(FaceHX(v0, v2, v3,v1)) == f_mp.end() &&
        f_mp.find(FaceHX(v0, v3, v1,v2)) == f_mp.end() &&
        f_mp.find(FaceHX(v0, v3, v2,v1)) == f_mp.end() &&
        f_mp.find(FaceHX(v1, v0, v2,v3)) == f_mp.end() &&
        f_mp.find(FaceHX(v1, v0, v3,v2)) == f_mp.end() &&
        f_mp.find(FaceHX(v1, v2, v0,v3)) == f_mp.end() &&
        f_mp.find(FaceHX(v1, v2, v3,v0)) == f_mp.end() &&
        f_mp.find(FaceHX(v1, v3, v2,v0)) == f_mp.end() &&
        f_mp.find(FaceHX(v1, v3, v0,v2)) == f_mp.end() &&
        f_mp.find(FaceHX(v2, v0, v1,v3)) == f_mp.end() &&
        f_mp.find(FaceHX(v2, v0, v3,v1)) == f_mp.end() &&
        f_mp.find(FaceHX(v2, v1, v0,v3)) == f_mp.end() &&
        f_mp.find(FaceHX(v2, v1, v3,v0)) == f_mp.end() &&
        f_mp.find(FaceHX(v2, v3, v1,v0)) == f_mp.end() &&
        f_mp.find(FaceHX(v2, v3, v0,v1)) == f_mp.end() &&
        f_mp.find(FaceHX(v3, v0, v1,v2)) == f_mp.end() &&
        f_mp.find(FaceHX(v3, v0, v2,v1)) == f_mp.end() &&
        f_mp.find(FaceHX(v3, v1, v0,v3)) == f_mp.end() &&
        f_mp.find(FaceHX(v3, v1, v3,v0)) == f_mp.end() &&
        f_mp.find(FaceHX(v3, v2, v1,v0)) == f_mp.end() &&
        f_mp.find(FaceHX(v3, v2, v0,v1)) == f_mp.end()) {
        f_mp.insert(std::pair<FaceHX, int>(FaceHX(v0, v1, v2,v3), num_faces));
        num_faces++;
        FV.push_back({v0, v1, v2, v3});
      }
  };
//start to read elem
  fscanf(ele_fp, "%d %d %d", &_0, &_1, &_2);
  ...
  for (int i = 0; i < num_cells; ++i) {
    fscanf(ele_fp, "%d %d %d %d %d", &_0, &_1, &_2, &_3, &_4, &_5, &_6, &_7,&_8);
    if (shuffle) {
      _1 = perm[_1];
      _2 = perm[_2];
      _3 = perm[_3];
    ...
    add_edge(_6, _7);
    add_edge(_7, _8);
    add_face(_1,_2, _3, _4);
    add_face(_1, _2, _6, _5);
    add_face(_1, _5, _8, _4);
    add_face(_4, _8, _7, _3);
    add_face(_3, _7, _6, _2);
    add_face(_5, _6, _7, _8);
...
std::shared_ptr<Mesh> Mesh::load_mesh(std::string filename) {
  std::cout<<"patcher in main"<<std::endl;
  if (filename.substr(filename.size() - 3) == "obj") {
    return load_obj(filename, false);
  }
  else if (filename.substr(filename.size() - 7) == "hex") {//for hex mesh, the file name should be like "filename.hex.node" and "filename.hex.ele"  
    return load_hex(filename, false);
  }
  else {
