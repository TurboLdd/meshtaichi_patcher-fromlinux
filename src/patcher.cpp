...
Csr& Patcher::get_relation(int from_end, int to_end) {
    ...
    if (key[0] == key[1])
    {
        if (key[0] == 7)
        {
            relation[key] = get_relation(key[0], key[0] - 4).mul(get_relation(key[0] - 4, key[0])).remove_self_loop();
        }
        else if (key[0] == 3)
        {
            relation[key] = get_relation(key[0], key[0] - 2).mul(get_relation(key[0] - 2, key[0])).remove_self_loop();
        }
        else
        {
    ...
    for (int i = 0; i < to.size(); i++) {
        ...
        sort(k.begin(), k.end());
        ele_dict[k] = i;//return the row index of to, that is map the vertex index to the row index ，即由k里面的顶点组成是哪个面 
    }
    vector<int> offset, value;
    auto &from = relation[{from_end, 0}];//3-0 c-v
    offset.push_back(0);
    for (int u = 0; u < from.size(); u++) {
        vector<int> subset, k(from[u].begin(), from[u].end());//代表某个cell由哪些顶点组成
        sort(k.begin(), k.end());
        function<void(int)> fun = [&](int i)
        {
            ...
            if (subset.size() == to_end + 1)
            {
                ...
                if (ele_dict.find(temp) != ele_dict.end())
                {
            ...
            if (subset.size() + k.size() - i < to_end + 1)
                return;
            fun(i + 1);
...
Csr& Patcher::get_face() {
    auto i = relation.find({3, -1});
    if (i == relation.end()) {
        relation[{3, -1}] = Csr();//modi
    }
    return relation[{3, -1}];
}
